...
export default function MoreInfoModal({config}){
    ...
    const items = name === 'teacher' ?   groups : users.filter(user=>user.role === 'teacher') 
    const [dataSet,setDataset]= useState(name === 'teacher' ?  groups : items )
    ...
    const removeItem = (item)=>{
        ...
        setDataset(prev => prev.filter(v=> v[key] !== item[key]))
    ...
    <div className="relative p-4 w-full max-w-2xl mx-auto">
        <div className="relative bg-gray-50 dark:text-gray-50 text-gray-700 dark:bg-gray-800 rounded-lg shadow border dark:border-gray-500 px-3 py-2">
            <h1 className=" mb-4 text-lg font-medium">Manage the {name === 'group' ? 'teachers' : 'groups'} of  {selectedItem.libel || selectedItem.name}  </h1>
            <div className="min-h-96">
               <MultipleSelect setItems={setDataset} items={items} dataset={dataSet}/>
               <h1 className='text-sm font-medium mt-5 capitalize'>{name}s List : </h1>
                <div className="flex gap-2 items-center flex-wrap mt-3">
             {dataSet.map(v=>
              <span key={v.matricule || v.id} className="text-sm  px-3 py-2 borber-2 hover:bg-purple-100 border-purple-300 bg-purple-50 text-purple-700  dark:bg-purple-700 dark:text-purple-50 rounded-xl flex items-center gap-2 justify-between ">
              {v[key]} 
