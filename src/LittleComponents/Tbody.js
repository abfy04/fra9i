...
import { Trash2,SquareArrowOutUpRight,Edit,Expand,RefreshCcw, CalendarFold } from 'lucide-react'
...
export default function Tbody ({data,config,columns}){
    const {dropDown,resetPassword,links,profile,moreInfo,name,action} = config
  ...
  return  (
    ...
    {
      data.map((t,index) =>
          <tr key={`row_${index}`} className={`hover:bg-gray-100 even:bg-gray-50 text-gray-700 dark:text-gray-50 dark:even:bg-gray-900 dark:hover:bg-gray-600  `} >
              {
                ...
                keys.map (
                  key => Array.isArray(t[key]) ?
                   ...
                   { key === 'status' ? <span className={`px-3 py-1 border rounded-xl text-sm font-medium ${t.status === 'Absent' ? 'bg-red-200 text-red-900 border-red-600 dark:bg-red-200' : "bg-yellow-200 text-yellow-900 border-yellow-600 "}`}>{t.status}</span> 
                   :
                     ( key === 'justified' ? 
                     <span className={`px-3 py-1 border rounded-xl text-sm font-medium ${t.justified === 'Yes' ? 'bg-green-200 text-green-900 border-green-600 dark:bg-green-200' : "bg-yellow-200 text-yellow-900 border-yellow-600 "}`}> {t.justified}</span> :  t[key]) } 
                ...
                <td className="px-6 py-2  whitespace-nowrap text-end text-sm font-medium">
              {
                ...
                >
                     ...
                     {profile && <Link to={`/${links.profile}/${selectedItem?.cef || selectedItem?.id}`} className="rounded-sm hover:bg-gray-100 dark:hover:bg-gray-600 flex gap-2 items-center text-xs p-2"><SquareArrowOutUpRight size={16}/> See More</Link>}
                      <Link to={`/${links.edit}/${selectedItem?.cef || selectedItem?.matricule || selectedItem?.id}`} className="rounded-sm hover:bg-gray-100 dark:hover:bg-gray-600 flex gap-2 items-center text-xs p-2"><Edit size={16}/> Edit</Link>
